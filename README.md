...
2. **Raw Layer (Truncate & Load):**
The raw layer consists of two tables raw_pupil and raw_pupil_attendance. Both tables are loaded directly from the
source csv files (pupil_data.csv and attendance_data.csv) in each dag run. The raw layer does not keep data history, so each load is a full refreshe
based on the content of the csv files.
I decided to use a trunc and load because CSVs are full refreshes, so no deltas to worry about.
Pandas reads the CSVs and inserts via SQL.

3. **Staging Layer (Insert-Only):**
The raw layer consists of two tables stg_pupil and stg_pupil_attendance. Both tables are populated with data from
the raw tables in each dag run. The stage tables keep history and they're insert only, so no duplicates are kept in the table
Originally, i opted for md5 hashing but sqlite presented alot of limitations for MD5() or SHA256() for hashing rows, so i went for insert or ignore.
...
Assumption: Dimensions change rarely; we don't track history here (e.g., if a pupil's name changes, it updates in dim_pupil via stage changes).

5. **Fact Table (Append-Only):**
This is the attendance sheet. It is an append only table and no existing record is updated, new records are only inserted
...
**Incremtal load logic:**
While the CSV is a static data, i realized data was written into it on a daily basis so i modeled my
pipeline to run daily based on the snapshot date in the stg pupil table. This mimics a daily data ingestion model
1. Truncate raw tables
2. Load CSVs into raw_pupil & raw_pupil_attendance
3. Insert into stage → only new/changed pupil-day combos
4. Insert into dims → only new values
5. Insert into fact → only the NEXT day after last loaded
The fact load also has a fail safe logic. If it is the first load, meaning the fact table is empty, the code
looks at the dim_date table and picks the MIn date there, this is to ensure that our incremental load starts from the very begining.
However, if the fact table is not empty, the script fetches the MAX date in the fact table and then loads data for that date +1 day, so it'll always be a da=eltsa load of 1 day
...
How to use (UI):
DAGs → run_sql_adhoc → Trigger DAG
{"sql_file": "C:/Users/PC/Documents/newglobe-pupil-datamart/sql/count_checker.sql"}
Click Trigger

How to use (Terminal):
bashairflow dags trigger run_sql_adhoc \
